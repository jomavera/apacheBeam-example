...
class SplitAirportData(beam.DoFn):
    def process(self, element):
        ...
        return [{"i94port": str(iata_code), "municipality": str(municipality)}]
...
def ToRowImmigration(values):
    ...
    return beam.Row(
        ...
        if values["biryear"] != None in values.keys()
        else 0,
...
def ToRowAirport(values):
    ...
    return beam.Row(
        ...
        if "municipality" in values.keys()
        else str("NaN"),
    )
    ...
    with beam.Pipeline(options=options) as p:
        ...
        immigration_data = (
            ...
            >> beam.io.parquetio.ReadFromParquet(
                p.options.input_dir.get() + "data.parquet\*"
        ...
        cities_data = (
            ...
            | "Get average demographics per State"
            >> beam.CombinePerKey(AverageDictFn())
            | "Key to Column"
            >> beam.ParDo(OrganizeCityData()).with_output_types(CityData)
        ...
        airport_data = (
            ...
            | "Parse airport data"
            >> beam.ParDo(SplitAirportData()).with_output_types(AirportData)
